import {
  ...
  type LaboratoryVerificationFilters,
...
const JOURNAL_PATH = "/api/laboratory/verification-journal";
...
export type LaboratoryVerificationListResult =
  | { status: "ready"; records: LaboratoryVerificationRecord[] }
  | ErrorResult;
export type LaboratoryVerificationSaveResult =
  | { status: "ready"; record: LaboratoryVerificationRecord }
  | ErrorResult;

export async function requestLaboratoryVerificationJournal(
  filters: LaboratoryVerificationFilters = {},
  options: RequestOptions = {},
): Promise<LaboratoryVerificationListResult> {
  ...
  if (
    ...
    return invalidResponse(
      "Сервер вернул журнал верификаций в неподдерживаемом формате.",
    );
  }
...
export async function submitLaboratoryVerificationRecord(
  submission: LaboratoryVerificationSubmission,
  options: RequestOptions = {},
): Promise<LaboratoryVerificationSaveResult> {
  return readSaveResult(
    await requestJson(JOURNAL_PATH, "POST", submission, options),
  );
}

export async function correctLaboratoryVerificationRecord(
  id: string,
  submission: LaboratoryVerificationCorrection,
  options: RequestOptions = {},
): Promise<LaboratoryVerificationSaveResult> {
...
): LaboratoryVerificationSaveResult {
  if (result.status === "error") return result;
  if (!isRecord(result.payload) || !isJournalRecord(result.payload.record)) {
    return invalidResponse("Сервер не вернул сохранённую запись верификации.");
  }
  ...
  try {
    ...
    if (!response.ok) {
      return readRemoteError(
        payload,
        "Не удалось обработать журнал верификаций.",
      );
    }
