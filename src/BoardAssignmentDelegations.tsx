    ...
    {data && (data.assignments.length === 0 ? <p className="director-empty">На основе этого поручения ещё нет назначений сотрудникам.</p> :
      <div className="history-table-scroll"><ManagedTable tableId="board.delegations">
        ...
        <tbody>{data.assignments.map(item => <tr key={item.id}>
          ...
          <TableCell><details><summary>События: {item.comments.length}</summary><ol className="board-delegation-events">{[...item.comments].reverse().sort((a, b) => b.createdAt.localeCompare(a.createdAt)).map(comment => <li key={comment.id}>
