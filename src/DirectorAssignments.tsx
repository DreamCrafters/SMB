    ...
    {selected && !form && <section className="director-assignment-detail" ref={detailRef} tabIndex={-1} aria-labelledby="director-assignment-detail-title">
      ...
      {!history && selected.status !== "completed" && <section className="board-assignment-decision is-execute">
        ...
        <div className="board-assignment-dialog-actions">
          {(data.executableAssignmentIds?.includes(selected.id) ?? (!data.permissions.canManage && data.permissions.canExecute)) && <button type="button" className="secondary-button" disabled={saving || !comment.trim()} onClick={() => void mutate(() => directorRequest(`/api/director-assignments/${selected.id}/action`, "POST", { action: "record_progress", comment, revision: selected.revision }))}>Сохранить промежуточный результат</button>}
          {data.permissions.canManage && data.executableAssignmentIds?.includes(selected.id) && <button type="button" className="secondary-button" disabled={saving || !comment.trim()} onClick={() => void mutate(() => directorRequest(`/api/director-assignments/${selected.id}/action`, "POST", { action: "submit_for_review", comment, revision: selected.revision }))}>Отправить на проверку</button>}
