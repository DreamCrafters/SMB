...
for (const mode of ["send", "receive", "send-linked"]) {
  test(`director workspace ${mode} uses server permissions and offers the appropriate workflow`, async () => {
    ...
    let submitted;
    const employees = [{ id: mode === "send-linked" ? "person-linked" : "account:employee-1", userId: "employee-1", fullName: "Сотрудник первый", position: "Инженер", active: true }, { id: "account:employee-2", fullName: "Сотрудник второй", position: "Экономист", active: true }];
    const oldAssignment = { id: "old-assignment", number: "ГД-1", revision: 1, assignedOn: "2026-09-15", kind: "Поручение", summary: "Ранее назначенная задача", department: "", project: "", responsibleId: "account:employee-1", responsible: { ...employees[0], id: "account:employee-1" }, coExecutorIds: [], coExecutors: [], recurrence: "once", activeFrom: "2026-09-16", activeTo: "2026-09-16", currentOccurrenceDate: "2026-09-16", urgency: "", importance: "", progress: "", note: "", incomingNumber: "", completedOn: "", status: "in_progress", comments: [], documents: [], source: null, sourceBoardAssignmentId: null, postponedUntil: "" };
    globalThis.fetch = async (url, options = {}) => {
      assert.equal(new URL(String(url), "http://127.0.0.1:5173").pathname, "/api/director-assignments");
      if (options.method === "POST") submitted = JSON.parse(options.body);
      return new Response(JSON.stringify({ assignments: mode === "send-linked" ? [oldAssignment] : [], employees: mode !== "receive" ? employees : [], permissions: { canView: true, canManage: mode !== "receive", canExecute: mode === "receive", canManagePersonnel: false }, today: "2026-09-15" }), { headers: { "Content-Type": "application/json" } });
    };
    try {
      ...
      if (mode === "send-linked") {
        ...
        assert.equal(picker.value, "person-linked");
