 ...
 * отсортирован как история — от свежих к старым (строка без садки, затем по
 * убыванию даты садки, тай-брейкер `sequence_id`), а циклы одного номера идут
 * только вперёд во времени, значит первая встреченная запись номера и есть его
 * текущий цикл.
 */
