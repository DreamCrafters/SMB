...
test("equipment validation names invalid fields in Russian", () => {
  ...
  assert.deepEqual(result.errors, [
    ...
    "Строка 1, «Ремонт по мех. части»: введите число цифрами.",
  ]);
  assert.deepEqual(result.fieldErrors, [
    {
    ...
    },
    {
      ...
      message: "Строка 1, «Ремонт по мех. части»: введите число цифрами.",
    },
  ]);
...
test("scalar and section validation errors keep their visible context", () => {
  ...
  assert.deepEqual(firing, {
    ok: false,
    errors: [
      "Поле «Время прогонки, час(а)»: укажите число от 0 до 24.",
    ],
    fieldErrors: [{
  ...
  assert.deepEqual(cosh, {
    ok: false,
    errors: [
      "Поле «Загрузка, ковш/час»: укажите целое число от 0 до 1 000 000 000.",
      "Заполнение ж/д бункеров, строка 1, «Кол-во, т»: введите число цифрами.",
    ],
    fieldErrors: [
      {
  ...
  assert.deepEqual(precision, {
    ok: false,
    errors: [
      "Строка 1, «Годная, т по взвешиванию»: укажите не более трёх знаков после запятой.",
    ],
    fieldErrors: [{
