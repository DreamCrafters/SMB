 ...
 */
/**
 * Доработка задачи 95: стартовый перечень сырья с вкладки `Сырьё` таблицы
 * задач. Миграция переносит его один раз, дальше номенклатура накапливается в
 * системе, а Google Sheets для сырья больше не читается.
 */
export const initialRawMaterialNames = [
  ...
  "Глина М4",
  "Глина НУ-ПК",
  "Глина огнеупорная марки С",
  "Глина огнеупорная марки С (МЕЛКОЗЕРНИСТАЯ)",
  "Глина Ш-1",
  "Глина Ш-4",
  "Глиноземистое вяжущее (клинкер) тара МКР",
  "Графит ГТ-1 ф.1000",
  "Древесная мука М-400",
  "Заполнитель корундовый фракции 0-0,2 мм",
  "Зола рисовой лузги обож.КО-ЮПК-ЗРЛ (В)",
  "Кальцинированная сода (меш.25,0)",
  "КАМЦЕЛ-400 \"Стандарт\"",
  "Карбид кремния факционированный фр.325F",
  "Кварцевая мука марка Б, МКР",
  "Кварцевая мука R10 EW",
  "Кислота олеиновая",
  "Кислота ортофосфорная",
  "Клей марки КМТ",
  "Клеящий реагент Брикефикс",
  "Лигносульфонат технический 00002 порошкообразный",
  "Лом корунда",
  "Лом шамотных изделий",
  "Масса ММЛ-65 (брак)",
  "Материал высокоглиноземистый ШРС-75 (брикетированный)",
  "Материал высокоглиноземистый ШРС-75 (таренный)",
  "Мертель МШ-42",
  "Микросфера алюмосиликатная АСПМ-500",
  "Обточка чистая от кругов абразивных 25А",
  "Огнеупорлом (на благоустройство территории)",
  "Огнеупорлом (отлуп от футеровки вагонеток)",
  "Огнеупорлом дробленый",
  "Огнеупорлом новый ШГР-28",
  "Песок перлитовый вспученный М-100",
  "Песок перлитовый вспученный М-100 т",
  "Плавиковошпатовый конденсат ФФ-95",
  "порошок магнезитовый каустический (магнезит)",
  "Порошок теплоизолирующий марки КО-ЮПК (зола рисовой лузги)",
  "Сода кальцинированная техн., марка Б, мешок 25кг",
  "Сульфоалюминатнобелитовое вяжущее САБВ-30-1 МКР",
  "Теплоизолирующая смесь (2 компонентная)",
  "Теплоизолирующая смесь ТИС",
  "Теплоизолирующая смесь ТИС гранулированная",
  "Теплоизолирующая смесь USSC SIL MIX 011/1",
  "Теплоизолирующая смесь USSC SIL MIX 011/2",
  "Теплоизолирующая смесь USSC SIL MIX 011/2 c фасовкой",
  "ТИС 4.1 вариант 1",
  "ТИС 4.1 вариант 2",
  "Углеродно-магнезиальная смесь марки УМС",
  "УСМ класс 0-1",
  "УСМ класс 0-1 (антроцит)",
  "Цемент высокоглиноземистый ВГЦ 70 (тара по 50кг)",
  "Шлифзерно 53С F30",
  "Электрокорунд белый 25А ПСП ф-ия 0-0,045",
  "Электрокорунд белый 25А ПСП ф-ия 0-0,063",
  "Электрокорунд минус 1000",
] as const;
...
const migrations: Migration[] = [
  ...
  {
    /**
     * Доработка задачи 95: разовый перенос перечня сырья из таблицы задач.
     * Повтор наименования пропускается, поэтому уже добавленное в системе
     * сырьё миграция не трогает.
     */
  ...
  {
    id: "081_table_layouts",
    statements: [
      `create table if not exists app_table_layouts (
        table_id varchar(100) character set ascii collate ascii_bin not null primary key,
...
function buildInitialRawMaterialInsert() {
  const values = initialRawMaterialNames.map((name) => `(
    uuid(),
    ${sqlString(name)},
    ${sqlString(normalizeInitialProductBrandName(name))},
    'system-google-sheets-raw-material-import',
    'system-google-sheets-raw-material-import'
  )`).join(",\n");
