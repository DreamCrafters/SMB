...
В Vite dev-режиме, если backend не найден, временный dev-вход возвращается к локальным Vite endpoints, а диспетчерские формы и отправки временно работают через локальный тестовый `localStorage` текущего браузера. Интерфейс помечает такой режим как локальный тестовый. Для проверки реального хранения, live feed между устройствами и подготовки к серверу всё равно запускать PostgreSQL и backend по шагам выше.
