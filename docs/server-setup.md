...
- `GET /api/dispatcher/forms` — вернуть серверные определения 5 активных диспетчерских форм;
...
Для анкет из переданных Apps Script backend повторяет основные правила исходных скриптов: оборудование не принимает пустую строку отчёта, выводит месяц из даты, требует время простоя больше 0 при выбранной причине простоя, требует выработку больше 0 при простое меньше 8 часов и обновляет существующую запись за тот же бизнес, `Дата отчета` и оборудование; открытие инцидента использует исходные списки типов/критичности и получает серверный номер `INC-год-номер`; закрытие инцидента требует поля закрытия; вход посетителя получает время входа на сервере и отклоняется, если этот посетитель уже вошёл и ещё не вышел; выход посетителя выбирается только из открытых входов и получает время выхода на сервере. Газовые формы скрыты из активного списка и больше не принимаются для новой отправки.
...
- `payload`;
- `summary`;
- `dedupe_key` для upsert отчётов оборудования;
- `status`;
- `submittedByAccountId`;
- `submittedAt`;
- `receivedAt`.

Старые колонки `period`, `metric_code`, `raw_value`, `comment` остаются в таблице для совместимости миграции, но новая рабочая модель пишет `form_id`, `payload` JSON и `summary`. Для `form_id='equipment'` backend также пишет `dedupe_key`, поэтому повторный `POST /api/dispatcher/submissions` с тем же `businessAccountId`, нормализованной `reportDate` и `equipment` обновляет существующую строку вместо создания дубля.
...
Текущие миграции создают таблицу `dispatcher_submissions` и добавляют уникальный `dedupe_key` для дневных отчётов оборудования. После `git pull` на Jino или другом сервере с включённым `RUN_MIGRATIONS_ON_START=true` миграция применится при рестарте backend; при ручном режиме запусти команду выше перед проверкой сайта.
...
8. Запись должна появиться из backend/БД; разделы `Оборудование`, `Инциденты` и `Посетители` должны обновиться из live feed.
...
- формы берутся из локальной тестовой копии 5 активных диспетчерских форм;
- отправки сохраняются в `localStorage` текущего браузера;
- вкладка владельца `Диспетчерская` читает эту же локальную историю и строит те же 3 раздела;
