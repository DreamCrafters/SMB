...
Для анкет из переданных Apps Script backend повторяет основные правила исходных скриптов: оборудование не принимает пустую строку отчёта, выводит месяц из даты и обновляет существующую запись за тот же бизнес, `Дата отчета` и оборудование; инцидент использует исходные списки типов/критичности и получает серверный номер `INC-год-номер`, закрытие инцидента требует поля закрытия, посетитель получает время входа на сервере.
