...
- временный dev-вход может вернуться к локальным Vite endpoints `/api/access/profile` и `/api/dev/access-session`;
...
Это не production-хранение и не проверка PostgreSQL. Для реальных данных, работы с другого устройства и подготовки к арендованному серверу запускать backend и БД по основной инструкции.
