...
В этом же листе должна быть колонка `Адресаты по инцидентам и оборуджованию (емейлы)`. Также поддерживается написание без опечатки: `Адресаты по инцидентам и оборудованию (емейлы)`.
...
- строки `22–25` — дополнительные адресаты, если в отчёте по оборудованию причина простоя относится к механической части;
- строки `27–30` — дополнительные адресаты, если в отчёте по оборудованию причина простоя относится к электрической части.

Если причина простоя содержит и `мех`, и `эл`, уведомление уйдёт обеим специализированным группам. Дубли адресов удаляются перед отправкой.
