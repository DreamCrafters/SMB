...
Администратор вставляет ссылку и сначала нажимает `Проверить таблицу`. Backend автоматически привязывает импорт к единственному активному бизнес-аккаунту; клиент не выбирает и не передаёт этот ID. Если активных бизнесов нет или их больше одного, импорт блокируется, чтобы данные не попали в неверный контур. Предпросмотр показывает количество исходных строк, создаваемых записей, уже существующих записей, пропусков и предупреждений. До кнопки `Перенести в БД` база не меняется. Если книга изменилась после предпросмотра, backend отклоняет перенос и требует проверить её заново.
...
Повторный перенос безопасен: одинаковые записи схлопываются по нормализованному содержимому внутри одной книги ещё до записи в БД. `import_source_key` защищает повторный импорт того же источника, а общий для импорта и обычных диспетчерских форм business-scoped `dedupe_key` не допускает такую же запись из копии книги, параллельного импорта или одновременной live-отправки. Для оборудования совпадением считаются бизнес, дата отчёта и оборудование; для открытия/закрытия инцидента — бизнес, тип записи и номер инцидента; для входа/выхода посетителя — бизнес, тип записи, соответствующее время, ФИО и организация. `insert ignore` не перезаписывает уже сохранённые production-строки. Миграция `014_dispatcher_spreadsheet_import_source` должна быть применена до первого переноса.
