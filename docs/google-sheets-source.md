...
Для таблиц огнеупорного цеха используются отдельные колонки `Адресаты ОЦ (емейлы)` и `Адресаты ОЦ (МАКС)`. Backend читает из них строки `2–20` и отправляет полное содержимое конкретной таблицы и рассчитанные итоги только после её подтверждения диспетчером. Длинная таблица для MAX автоматически делится на нумерованные части без обрезания строк. Возврат на доработку остаётся внутренним уведомлением аккаунта ОЦ и этим адресатам не рассылается.
...
Рассылка выполняется только после успешного сохранения формы либо подтверждения таблицы ОЦ в БД. Если SMTP временно недоступен, данные остаются сохранёнными, а ошибка записывается в лог backend как `dispatcher_notifications.email_send_failed` или `refractory_notifications.email_send_failed`.
...
Рассылка выполняется только после успешного сохранения формы либо подтверждения таблицы ОЦ в БД. Если MAX временно недоступен или токен неверный, данные остаются сохранёнными, а ошибка записывается в лог backend как `dispatcher_notifications.max_send_failed` или `refractory_notifications.max_send_failed`. Если подходящая форма сохранена, но в Google Sheets не найдено ни одного MAX ID, backend пишет `dispatcher_notifications.max_no_recipients` или `refractory_notifications.max_no_recipients`.
