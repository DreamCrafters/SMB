...
Администратор вставляет ссылку, выбирает целевой бизнес-аккаунт и сначала нажимает `Проверить таблицу`. Предпросмотр показывает количество исходных строк, создаваемых записей, уже существующих записей, пропусков и предупреждений. До кнопки `Перенести в БД` база не меняется. Если книга изменилась после предпросмотра, backend отклоняет перенос и требует проверить её заново.
