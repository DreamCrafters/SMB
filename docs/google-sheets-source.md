...
Повторный перенос безопасен: `import_source_key` и существующий equipment key делают его идемпотентным, а `insert ignore` не перезаписывает уже сохранённые production-строки. Миграция `014_dispatcher_spreadsheet_import_source` должна быть применена до первого переноса.
