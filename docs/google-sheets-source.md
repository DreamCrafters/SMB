...
Отчёт по оборудованию рассылается одним сообщением на весь дневной пакет, который backend получает через `/api/dispatcher/equipment-report`. Одиночная отправка одной позиции оборудования через общий endpoint не должна создавать email/MAX-уведомление.
