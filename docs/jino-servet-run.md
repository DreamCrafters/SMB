...
откроет локальный терминал с `ssh-copy-id`. В появившемся запросе нужен пароль
SSH-аккаунта Jino `j53403317`, а не пароль Git или VS Code. После успешного
копирования ключа вручную обновите статусы из того же меню.
